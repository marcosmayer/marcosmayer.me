...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($result["success"] && $result["score"] >= 0.5 && $result["action"] == "submit") {
		...
		$palestraSelecionada = isset($_POST['palestraSelecionada']) ? $_POST['palestraSelecionada'] : [];
		$palestrasInteresse = implode(", ", $palestraSelecionada);

		// Se todos os campos são válidos, envie o e-mail
		if ($isValid) {
			$to = "marcos@marcosmayer.me";
			$subject = "Nova solicitação de palestra";
			$body = "Nome: $nomeCompleto\nEmail: $email\nCelular: $celular\nEmpresa: $empresa\nData prevista: $dataPalestra\nLocal: $local\nPúblico previsto: $publico\nOrçamento previsto: $orcamento\nPalestras de Interesse: $palestrasInteresse\nMensagem: $mensagem\n";
			$headers = "From: webmaster@marcosmayer.me\r\n";
			$headers .= "MIME-Version: 1.0\r\n";
			$headers .= "Content-type: text/plain; charset=UTF-8\r\n";

			if (mail($to, $subject, $body, $headers)) {
				header("Location: ../pages/palestras/sucesso.php");
				exit();
			} else {
